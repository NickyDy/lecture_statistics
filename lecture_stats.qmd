---
title: "Анализ на данни, визуализация и тестване на хипотези."
...
9.  Отстраняване на нежелани стойности още при импортирането.
...
```{r}
diss %>% filter(elev > 1500)

diss %>% filter(elev > 1500 & top == "flat")

diss %>% filter(top == "concave" | exp == "e_nw")

diss %>% filter(com %in% c("a", "b"))

diss_up_to_1000 <- diss %>% filter(!elev > 1500)
```
...
![Какво се случва при удължаването на данните [@wickham2016r]](longering.png){alt="Какво се случва при удължаването на данните [@wickham2016r]"}
...
2.  Прогнозиране на възрастта от ДНК метилация [@akalin2020computational].
